...
function renderNotes(){
 ...
 const arr=notes.filter(n=>!f||n.category===f).sort((a,b)=>String(b.updatedAt||'').localeCompare(String(a.updatedAt||'')));
 $('#notesGrid').innerHTML=arr.map(n=>`<article class="note-card" data-id="${esc(n.id)}" style="background:${esc(n.color||'#fff4cc')}"><h3><span class="title">${esc(n.title||tr('untitled'))}</span><span class="category">${esc(n.category||'')}</span></h3>${n.text?`<p>${esc(n.text)}</p>`:''}${(n.checklist||[]).slice(0,4).map(c=>`<div class="mini-check"><input type="checkbox" ${c.done?'checked':''} disabled><span>${esc(c.text)}</span></div>`).join('')}${n.image?`<img src="${n.image}" alt="">`:''}</article>`).join('')||`<div style="color:var(--muted);padding:24px;text-align:center">${esc(tr('empty'))}</div>`;
...
function addCheckRow(item={text:'',done:false}){const d=document.createElement('div');d.className='check-item';d.innerHTML=`<input type="checkbox" ${item.done?'checked':''}><input type="text" value="${esc(item.text)}"><button type="button" aria-label="Delete">×</button>`;d.querySelector('button').onclick=()=>d.remove();$('#checkList').appendChild(d)}
function openNote(id=null){
 editing=id;const n=notes.find(x=>x.id===id)||{title:'',text:'',color:'#fff4cc',category:cats[0],checklist:[],image:''};
 $('#editorTitle').textContent=id?tr('editNote'):tr('newNote');$('#noteTitle').value=n.title||'';$('#noteText').value=n.text||'';$('#noteColor').value=n.color||'#fff4cc';renderCats();$('#noteCategory').value=cats.includes(n.category)?n.category:cats[0];$('#checkList').innerHTML='';(n.checklist||[]).forEach(addCheckRow);$('#checkText').value='';$('#noteImage').value='';draftImage=n.image||'';renderImage();$('#deleteNote').style.visibility=id?'visible':'hidden';$('#editor').classList.remove('hidden');$('#editor').setAttribute('aria-hidden','false');setTimeout(()=>$('#noteTitle').focus(),0)
}
...
$('#filterCategory').onchange=renderNotes;$('#clearNoteFilter').onclick=()=>{$('#filterCategory').value='';renderNotes()};
$('#languageSetting').onchange=()=>{settings.language=$('#languageSetting').value;persist();renderAll()};
